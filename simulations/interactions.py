...
def birth(lgca):
    ...
    :return:
    """
    lgca.r_b = 0.1 # randomly set by me - i think it's birth rate
...
def chemotaxis(lgca):
    ...
    newnodes = lgca.nodes.copy() # new nodes is a copy of old nodes
    relevant = (lgca.cell_density[lgca.nonborder] > 0) & \
               (lgca.cell_density[lgca.nonborder] < lgca.K) # less than max node capacity
    coords = [a[relevant] for a in lgca.nonborder]
    for coord in zip(*coords):
    ...
    lgca.nodes = np.add(lgca.nodes.copy(), newnodes)
    print("step")
    # lgca.r_b = 0.7
    # birth = npr.random(lgca.nodes.shape) < lgca.r_b * lgca.cell_density[..., None] / lgca.K
    # lgca.nodes = np.add(newnodes.astype(int), lgca.nodes.astype(int) * birth)
